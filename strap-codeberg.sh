...
# ==============================================================================
# 脚本功能说明 (Bootstrap Script for Shorin Arch Setup - Codeberg Edition)
...
# 4. 流式处理：通过 Codeberg 拉取源码，pv 提供带有预估总量的真实进度条与网速监控。
...
# 【极客魔法】预估源码压缩包体积。实际测得约为 60MB，这里预留一点余量设定为 80M，确保进度条平滑且不卡顿。
EXPECTED_SIZE="80M"

printf "%b>>> Preparing to install from Codeberg branch: %s on %s%b\n" "$BLUE" "$TARGET_BRANCH" "$ARCH_NAME" "$NC"
...
printf "Downloading and extracting repository from Codeberg to %s...\n" "$TARGET_DIR"
...
for attempt in 1 2 3; do
    ...
    if [ "$attempt" -eq 3 ]; then
        printf "%bError: Failed to download Codeberg branch '%s' after 3 attempts. Network issue suspected.%b\n" "$RED" "$TARGET_BRANCH" "$NC"
        exit 1
    fi
